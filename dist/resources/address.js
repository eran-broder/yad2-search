...
export const createAddressResource = (gateway) => {
     ...
     */
    const locateEnglish = async (text) => {
        const wanted = normalize(text);
        const cities = await list(AddressPath.Cities, CitySchema);
        return (cities.find((city) => city.city_eng !== undefined && normalize(city.city_eng) === wanted) ??
     ...
     */
    const nearbySpellings = async (text) => {
        ...
        return cities
            ...
            .slice(0, MAX_HINTS)
            .map((city) => `${city.city_eng} (${city.city_heb})`);
    };
