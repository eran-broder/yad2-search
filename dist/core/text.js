...
export const normalizeName = (text) => text.replace(MARKS, '').replace(SEPARATORS, ' ').replace(WHITESPACE, ' ').trim().toLowerCase();
/**
 ...
 */
export const findByName = (entries, name) => {
    const wanted = normalizeName(name);
    if (wanted.length === 0)
        return undefined;
    const namesOf = (entry) => [entry.heb, entry.eng].filter((value) => Boolean(value)).map(normalizeName);
