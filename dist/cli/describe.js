...
const describeType = (schema, depth = 0) => {
    ...
    switch (def.type) {
        ...
        case ZodTypeName.Array:
            return `${describeType(def.element, depth + 1)}${ARRAY_SUFFIX}`;
        case ZodTypeName.Union: {
            ...
            const collapsed = parts.filter((p) => !(p.endsWith(ARRAY_SUFFIX) && scalars.includes(p.slice(0, -ARRAY_SUFFIX.length))));
...
const isOptional = (schema) => defOf(schema).type === ZodTypeName.Optional;
export const describeParams = (fn) => {
    const schema = paramsOf(fn);
...
export const describeSignature = (fn, path) => {
    ...
    if (params.length) {
        ...
        const described = positional.find((name) => name === PARAMS_ARGUMENT) ?? positional[0];
        lines.push('', `${described}:`, ...params.map((p) => `  ${p}`));
        lines.push('', `  * = required.  e.g. node dist/cli.js ${path} '{"…":…}'`);
    }
    else {
        lines.push(`  e.g. node dist/cli.js ${path} ${positional.map((p) => `'"<${p}>"'`).join(' ')}`, `  See references/recipes.md for worked examples.`);
    }
