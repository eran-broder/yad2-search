...
const formatFrom = (flag: unknown): OutputFormat => {
  if (flag === undefined || flag === true) return OutputFormat.Json;
  if (isOutputFormat(flag)) return flag;
  throw new Yad2Error(
    `Unknown --format "${String(flag)}". Use ${Object.values(OutputFormat).join(', ')}.`,
  );
};
...
const main = async (): Promise<number> => {
  ...
  const shaped = wantsRows && toRows ? toRows(result as never) : result;
  ...
  if (missing.length) {
    console.error(
      `warning: no data at ${missing.join(', ')} — run the same command with --paths to list the real field names`,
    );
  }
